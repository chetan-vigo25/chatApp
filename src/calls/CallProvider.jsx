...
// into MODE_IN_COMMUNICATION. The module is platform-guarded and lazily requires
// the lib, so importing it here is safe on every engine/platform combination.
import * as AudioRoute from './native-engine/AudioRoute';
...
export const CallProvider = ({ children }) => {
  ...
  const applyInitialCallRoute = useCallback(() => {
    ...
    if (Platform.OS === 'android') {
      // ── Put Android into its CALL audio mode — do this FIRST ───────────────
      // InCallManager.start() is what sets MODE_IN_COMMUNICATION: the voice
      // audio path (small playout buffers), hardware AEC/NS/AGC, STREAM_VOICE_CALL
      // and call audio focus. Without it the WebView's WebRTC audio runs on the
      // MEDIA/playback path, whose deep buffers make latency ACCUMULATE (audio
      // arriving seconds late) while the software AEC fights the missing hardware
      // one (choppy/robotic). That is the whole reason the same call is smooth in
      // Chrome and broken in the app: Chrome's WebRTC stack sets this mode itself,
      // the embedded Android System WebView does not — the host app must.
      //
      // Previously this ran ONLY on the native engine (NativeCallEngine), which is
      // iOS-only today (CALL_NATIVE_ENGINE_ANDROID = false), so Android never got
      // a call audio session at all. See the TODO(audit APP-12) note in
      // engine/callEngineHtml.js and PHASE_1_AUDIT.md §57.
      //
      // ONE owner of the global audio state. When InCallManager is present it
      // owns BOTH the mode and the routing; expo-av must not also drive it —
      // setAudioModeAsync re-applies its own mode on every call and would fight
      // MODE_IN_COMMUNICATION (the same two-owners hazard the iOS notes below
      // describe, where each reconfigure killed the WebView's live mic track).
      // expo-av stays only as the FALLBACK for a build where the native module
      // isn't linked, so the Speaker button can never become a no-op.
      AudioRoute.start({ video: stateRef.current.media === 'video' });
      if (AudioRoute.isAvailable()) AudioRoute.setSpeaker(on);
      else applyAudioRoute(on);
      // Either way the engine still needs setSinkId: that re-routes the WebView's
      // WebRTC <audio> element, which is what actually carries the call audio.
  ...
  const resetAudioRoute = useCallback(async () => {
    ...
    // Release the call audio session (Android WebView engine — the iOS native
    // engine stops it from its own hangup path). This MUST run before the
    // `audioRouteAppliedRef` early-return further down: AudioRoute keeps a
    // module-level `started` flag that only stop() clears, so a skipped stop
    // makes the NEXT call's start() a silent no-op — i.e. one missed teardown
    // would permanently drop us back to the broken media-path audio.
    if (Platform.OS === 'android') AudioRoute.stop();
    if (Platform.OS === 'ios') {
  ...
  const upgradeUiToVideo = useCallback((selfCamera) => {
    ...
    if (snap.media !== 'video') {
      ...
      if (!snap.speakerOn) {
        ...
        // Single-owner rule (see applyInitialCallRoute): expo-av must not re-apply
        // its own audio mode mid-call once InCallManager holds the session.
        if (Platform.OS === 'android') {
          if (AudioRoute.isAvailable()) AudioRoute.setSpeaker(true);
          else applyAudioRoute(true);
        }
  ...
  const toggleSpeaker = useCallback(() => {
    ...
    if (Platform.OS === 'android') {
      ...
      // Route through whichever owns the session — same single-owner rule as
      // applyInitialCallRoute (expo-av only when InCallManager isn't linked).
      if (AudioRoute.isAvailable()) AudioRoute.setSpeaker(next);
      else applyAudioRoute(next);
  ...
  const reassertSpeakerRoute = useCallback(() => {
    ...
    if (Platform.OS === 'android') {
      // Re-assert through the session owner. Also re-arms the call audio mode if
      // something (a telephony interruption, another app grabbing focus) knocked
      // us back to MODE_NORMAL mid-call — AudioRoute.start() is guarded, so this
      // only re-applies routing when the session is already up.
      if (AudioRoute.isAvailable()) AudioRoute.setSpeaker(on);
      else applyAudioRoute(on);
      sendCmd({ cmd: CMD.SET_SPEAKER, on });
    } else if (isNativeCallEngine()) {
