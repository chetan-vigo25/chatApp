 ...
 *
 * @param {boolean} enabled  gate streaming on auth/socket readiness
 */
export const useLocationTracking = (enabled = true) => {
  ...
  useEffect(() => {
    ...
    const emitFix = async (loc) => {
      ...
      // Keep the app-wide warm cache fresh — the call path (`call:ring`
      // location stamp) reads this instead of doing its own GPS round-trip.
    ...
    const start = async () => {
      // Shared in-context gate — re-asks the OS if location was denied at startup.
      // Silent: this is passive telemetry, so a refusal just disables streaming
      // rather than interrupting the user with an alert.
      const locationOk = await ensurePermission(PERMISSION_IDS.LOCATION, { silent: true });
      if (!locationOk || cancelled) return;
