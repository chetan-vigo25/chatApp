...
export default function Otp({ navigation, route }) {
    ...
    const handleVerifyOtp = async () => {
      ...
      try {
        ...
        // Update AuthContext with user data and tokens
        await authLogin(loginData.data, {
    ...
    <Animated.View style={{ flex: 1, opacity: fadeAnim, backgroundColor: theme.colors.background }}>
      <View style={{ flex: 1, paddingHorizontal: 20 }}>
        <View style={{ marginTop: 40 }}>
          <Text style={{ textAlign: 'center', fontFamily: 'Poppins-Medium', fontSize: 18, color: theme.colors.themeColor, paddingVertical:10 }} >
            Verify {selectedCountry?.code} {phoneNumber}
          </Text>
        </View>
        <View>
          <Text style={{ textAlign: 'center', fontFamily: 'Poppins-Medium', fontSize: 14, color: theme.colors.placeHolderTextColor }} >
            Waiting for OTP to be sent {selectedCountry?.code} {phoneNumber}.
          </Text>
          <TouchableOpacity onPress={() => navigation.navigate('Login')}>
            <Text style={{ textAlign: 'center', fontFamily: 'Poppins-Medium', fontSize: 14, color: theme.colors.themeColor }} >
              Wrong number?
            </Text>
          </TouchableOpacity>
        </View>
        ...
        <Text style={{ textAlign: 'center', fontFamily: 'Poppins-Medium', fontSize: 14, color: theme.colors.placeHolderTextColor, marginVertical:10 }} >
          Enter OTP
        </Text>
        <View style={{ width:'100%', flexDirection:'row', borderBottomWidth:1, borderColor: theme.colors.borderColor, paddingHorizontal:10 }} >
          ...
          >
            <Text style={{ 
              ...
              fontSize: 14, 
              color: seconds > 0 ? theme.colors.placeHolderTextColor : theme.colors.themeColor, 
              paddingBottom:5 
            }}>
              Resend SMS ?
            </Text>
          ...
          <View style={{ flex:1, justifyContent:'center', alignItems:'flex-end' }} >
            <Text style={{ 
              fontFamily: 'Poppins-Medium', 
              fontSize: 14, 
              color: seconds > 0 ? theme.colors.themeColor : theme.colors.placeHolderTextColor 
            }}>
              {formatTime(seconds)}
            </Text>
          </View>
        ...
        <TouchableOpacity 
          ...
          style={{ 
            width:'50%', 
            height:40, 
            backgroundColor: (isLoading || seconds <= 0) ? theme.colors.placeHolderTextColor : theme.colors.themeColor, 
            justifyContent:'center', 
            alignItems:'center', 
            alignSelf:'center', 
            marginTop:20, 
        ...
        >
          {isLoading ? (
            ...
            <Text style={{ fontFamily: 'Poppins-Medium', fontSize: 16, color: theme.colors.textWhite }} >
              Verify OTP
            </Text>
          )}
        </TouchableOpacity>
