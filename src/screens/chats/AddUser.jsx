...
useEffect(() => {
  ...
  const handleContactPress = async (contact) => {
    ...
    const existingChat = chatsData?.find(
      chat => chat.peerUser?._id === normalizedUser?.userId || chat.peerUser?._id === normalizedUser?.id
    );
  ...
  const renderContactRow = (contact, index, showInvite = false) => {
    ...
    return (
      ...
      >
        <TouchableOpacity onPress={() => openProfilePreview(contact)} style={{ width:50, height:50, borderRadius:100, alignItems:'center', justifyContent:'center', overflow:'hidden' }} >
          {
