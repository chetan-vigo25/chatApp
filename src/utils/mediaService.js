...
export async function copyToAppFolder(inputUri, suggestedName = null, destDir = SENT_DIR, saveToLibrary = true, onProgress = null) {
  try {
    ...
    if (typeof saveToLibrary === 'function' && onProgress == null) {
      onProgress = saveToLibrary;
      saveToLibrary = true;
    }
...
export async function downloadAndOpenMedia({ msg, dispatch, downloadAction, onProgress = null, openAfterDownload = false, saveToLibrary = true }) {
  try {
    ...
    if (!remoteUrl && downloadAction && dispatch) {
      ...
      remoteUrl = action?.payload?.data?.downloadUrl || action?.payload?.data?.url || action?.payload?.url || null;
    }
...
export async function persistDownloadedMedia({ mediaId, chatId, sourceUrl, fileName, messageType = 'file', onProgress = null }) {
  ...
  const baseDir = type === 'image'
    ? `${MEDIA_DIR}images/${chatId || 'general'}/`
    : type === 'video'
      ? `${MEDIA_DIR}videos/${chatId || 'general'}/`
      : `${MEDIA_DIR}documents/${chatId || 'general'}/`;

  await ensureDirExists(baseDir);

  const safeFileName = fileName || `${mediaId}`;
